...
[ApiController]
public class TodosController(AppDbContext context) : ControllerBase //primary constructor
{
    ...
    public IActionResult Add(AddTodoDto request)
    {
        Todo todo = new()
        {
            Work = request.Work.Trim(),//Trim() metodu string ifadelerin başındaki ve sonundaki boşlukları siler.
    ...
    public IActionResult ChangeCompletedStatus(int id)
    {
        ...
        todo.DateCompleted = todo.IsCompleted == false ? null : DateTime.Now; //todo.IsCompleted == false ise null, true ise DateTime.Now
